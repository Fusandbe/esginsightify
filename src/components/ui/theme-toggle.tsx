  ...
  if (variant === "toggle") {
    return (
      <Toggle
        ...
        className="data-[state=on]:bg-muted rounded-full w-9 h-9 p-0 flex items-center justify-center border border-primary/20"
      >
        {theme === "dark" ? 
          <Moon className="h-4 w-4 text-esg-gold-500 transition-all duration-300 rotate-0" /> : 
          <Sun className="h-4 w-4 text-primary transition-all duration-300 rotate-90" />
        }
      </Toggle>
    );
  }
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        ...
        >
          ...
          <Moon className="absolute h-[18px] w-[18px] transition-all duration-500 rotate-90 scale-0 dark:rotate-0 dark:scale-100 text-esg-gold-500" />
          <span className="sr-only">Toggle theme</span>
        </Button>
      </DropdownMenuTrigger>
      <DropdownMenuContent align="end" className="backdrop-blur-xl bg-background/95 border-primary/10 shadow-luxury rounded-lg">
        <DropdownMenuItem onClick={() => setTheme("light")} className="gap-2 cursor-pointer focus:bg-accent/5">
          <Sun className="h-4 w-4 text-primary" />
          <span className="font-serif">Light</span>
        </DropdownMenuItem>
        <DropdownMenuItem onClick={() => setTheme("dark")} className="gap-2 cursor-pointer focus:bg-accent/5">
          <Moon className="h-4 w-4 text-esg-gold-500" />
          <span className="font-serif">Dark</span>
        </DropdownMenuItem>
        <DropdownMenuItem onClick={() => setTheme("system")} className="gap-2 cursor-pointer focus:bg-accent/5">
          ...
          <span className="font-serif">System</span>
        </DropdownMenuItem>
      </DropdownMenuContent>
    </DropdownMenu>
  );
